...
def generate_deterministic_k(secexp,hash,hash_func):
    ...
    k_1 = hmac.new(k_0, v_0 + b'\x00' + bytearray.fromhex(hex(secexp)[2:]) + hash, hash_func).digest()
    v_1 = hmac.new(k_1, v_0, hash_func).digest()
    k_2 = hmac.new(k_1, v_1 + b'\x01' + bytearray.fromhex(hex(secexp)[2:]) + hash, hash_func).digest()
    v_2 = hmac.new(k_2, v_1, hash_func).digest()
